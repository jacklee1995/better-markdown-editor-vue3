...
export class MermaidPlugin extends BasePlugin {
  ...
  public tokenizer(src: string): Token[] {
    ...
    while ((match = rule.exec(src))) {
      const code = match[1].trim() // 获取 Mermaid 代码内容
  ...
  public renderer(
    ...
    const id = `mermaid-${idx}` // 为每个 Mermaid 图表生成唯一的 ID
    return `<div id="${id}" class="mermaid-container"></div>\n<script>window.addEventListener("load", () => { mermaid.initialize({ startOnLoad: false }); mermaid.render("${id}", '${code}') })</script>\n`
  }
