import { BasePlugin } from '../BasePlugin'
import type { MarkdownParserOptions, Token, Renderer } from '../../markdown/interface'

/**
 * 脚注插件
 * 支持通过 [^1] 引用和 [^1]: 定义语法创建脚注
 */
export class FootnotePlugin extends BasePlugin {
  public name = 'footnote'
  public level: 'block' | 'inline' | 'core' = 'inline'

  /**
   * 分词器
   * @param src 源码
   * @returns Token 数组
   */
  public tokenizer(src: string): Token[] {
    const refRule = /\[\^(\d+)\]/g // 匹配脚注引用
    let match: RegExpExecArray | null
    const tokens: Token[] = []

    // 处理脚注引用
    while ((match = refRule.exec(src))) {
      const id = match[1]

      tokens.push({
        type: 'footnote_ref',
        raw: match[0],
        text: id,
        block: false,
        depth: 0,
        attrGet: () => null,
        attrSet: () => {},
        attrJoin: () => {},
        children: [],
        map: [0, 0],
      })
    }

    const defRule = /\[\^(\d+)\]:\s*([\s\S]+?)(?=\n\n)/g // 匹配脚注定义

    // 处理脚注定义
    while ((match = defRule.exec(src))) {
      const id = match[1]
      const content = match[2].trim()

      tokens.push({
        type: 'footnote_def',
        raw: match[0],
        text: content,
        block: true,
        depth: 0,
        attrGet: (name: string) => {
          if (name === 'id') return id
          return null
        },
        attrSet: () => {},
        attrJoin: () => {},
        children: [],
        map: [0, 0],
      })
    }

    return tokens
  }

  /**
   * 渲染器
   * @param tokens Token 数组
   * @param idx 当前 Token 索引
   * @param options 解析选项
   * @param env 环境变量
   * @param self 渲染器实例
   * @returns 渲染后的 HTML 字符串
   */
  public renderer(
    tokens: Token[],
    idx: number,
    options: MarkdownParserOptions,
    env: Record<string, unknown>,
    self: Renderer,
  ): string {
    const token = tokens[idx]
    if (token.type === 'footnote_ref') {
      const id = token.text
      return `<sup class="footnote-ref"><a href="#fn${id}">[${id}]</a></sup>`
    } else {
      const id = token.attrGet('id')
      const content = token.text
      const contentToken: Token = {
        type: 'text',
        raw: content,
        text: content,
        block: false,
        depth: 0,
        attrGet: () => null,
        attrSet: () => {},
        attrJoin: () => {},
        children: [],
        map: [0, 0],
      }
      return `<div class="footnote" id="fn${id}"><sup>${id}</sup>. ${self.render([contentToken], options, env)}<a href="#fnref${id}">↩</a></div>`
    }
  }

  /**
   * 插件开始处理前的钩子函数
   * @param src 源码
   * @returns 处理后的源码
   */
  public start(src: string): string {
    return src
  }
}
